...
class UserUseCase {
	private readonly userRepository: UserRepository = new UserRepositoryPrisma();
	...
	async updateByClerk({
		...
		lastName,
		email,
	}: UserUpdateByClerk): Promise<UserUpdateByClerk> {
		if (!externalId) {
			throw new Error("External ID is required");
		}
		const user = await this.userRepository.findUserByExternalId(externalId);
		return await this.userRepository.userUpdateByClerk({
			id: user.id,
			firstName,
			lastName,
			email,
