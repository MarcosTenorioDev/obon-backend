...
} from "../interfaces/eventCategory.interface";

class EventCategoryRepositoryPrisma implements EventCategoryRepository {
	async createEvent(
		eventCategoryCreate: EventCategoryCreate
		...
		try {
			...
			return await prisma.eventCategory.create({
				data: {
					...
					isActive: eventCategoryCreate.isActive ?? true,
				},
			});
		} catch (error) {
			console.log(error);
			if (
				...
				error.message === "CategoryAlreadyExistsError"
			) {
